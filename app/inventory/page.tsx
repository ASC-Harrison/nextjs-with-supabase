...
export default function InventoryPage() {
  ...
  useEffect(()=>{try{setLocked(!getSessionUnlocked());const savedArea=localStorage.getItem(LS.AREA);if(savedArea)setAreaId(savedArea);setStaffName(localStorage.getItem(LS.STAFF)||"");setAudit(safeJsonParse<AuditEvent[]>(localStorage.getItem(LS.AUDIT),[]));setLastTx(safeJsonParse<LastTx|null>(localStorage.getItem(LS.LAST_TX),null));}catch{}},[]);// eslint-disable-line
  ...
  async function doTotalsSet(row:BuildingTotalRow,value:number,pinAlreadyPassed=false){if(!staffName.trim()){setTab("Audit");alert("Enter staff name in Audit tab first.");return;}if(locked&&!pinAlreadyPassed){setPendingTotalsAction({kind:"SET",value});openPin("totalsEdit");return;}const res=await fetch("/api/building-inventory/update",{method:"POST",headers:{"Content-Type":"application/json"},cache:"no-store",body:JSON.stringify({item_id:row.item_id,action:"SET",value})});const json=await res.json();if(!json.ok){alert(`Update failed: ${json.error}`);return;}pushAudit({action:"TOTALS_SET",details:`Item=${row.name} Set=${value}`});setTotalsEditOpen(false);await loadTotals();}
  async function doTotalsAdjust(row:BuildingTotalRow,delta:number,pinAlreadyPassed=false){if(!staffName.trim()){setTab("Audit");alert("Enter staff name in Audit tab first.");return;}if(delta===0){alert("Delta cannot be 0.");return;}if(locked&&!pinAlreadyPassed){setPendingTotalsAction({kind:"ADJUST",delta});openPin("totalsEdit");return;}const res=await fetch("/api/building-inventory/update",{method:"POST",headers:{"Content-Type":"application/json"},cache:"no-store",body:JSON.stringify({item_id:row.item_id,action:"ADJUST",delta})});const json=await res.json();if(!json.ok){alert(`Update failed: ${json.error}`);return;}pushAudit({action:"TOTALS_ADJUST",details:`Item=${row.name} Delta=${delta>0?"+":""}${delta}`});setTotalsEditOpen(false);await loadTotals();}
  async function doTotalsSetActive(row:BuildingTotalRow,is_active:boolean,pinAlreadyPassed=false){if(!staffName.trim()){setTab("Audit");alert("Enter staff name in Audit tab first.");return;}if(locked&&!pinAlreadyPassed){setPendingTotalsAction({kind:"SET_ACTIVE",is_active});openPin("totalsEdit");return;}if(!confirm(is_active?`Restore "${row.name}" to active items?`:`Move "${row.name}" to inactive items?`))return;const res=await fetch("/api/building-inventory/update",{method:"POST",headers:{"Content-Type":"application/json"},cache:"no-store",body:JSON.stringify({item_id:row.item_id,action:"SET_ACTIVE",is_active})});const json=await res.json();if(!json.ok){alert(`Update failed: ${json.error}`);return;}pushAudit({action:is_active?"ITEM_RESTORED":"ITEM_INACTIVE",details:`Item=${row.name}`});setTotalsEditOpen(false);await loadTotals();}
  ...
  async function saveItemStatus(itemId:string,order_status:string,backordered:boolean,itemName?:string){if(!staffName.trim()){setTab("Audit");alert("Enter staff name in Audit tab first.");return;}if(locked){setPendingItemStatusSave({item_id:itemId,order_status,backordered,item_name:itemName});openPin("itemStatusEdit");return;}await doSaveItemStatus(itemId,order_status,backordered,itemName);}
                  ...
                  {item && (
                    <div className="found-panel">
                      ...
                      <div style={{background:"rgba(59,130,246,0.08)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:"var(--r-lg)",padding:"14px",marginBottom:12}}>
                        ...
                        <button className="btn btn-submit btn-full btn-lg" style={{marginTop:12}} disabled={locked||staffMissing} onClick={submit}>{mode==="USE"?`USE ${qty} — Submit`:`RESTOCK ${qty} — Submit`}</button>
                        {(locked||staffMissing) && <div style={{fontSize:11,color:"var(--text4)",marginTop:6,textAlign:"center"}}>{staffMissing?"Set staff name in Audit tab first.":"Unlock to submit."}</div>}
                      </div>
        ...
        <AscModal title="Edit building totals" okText="Close" onCancel={()=>setTotalsEditOpen(false)} onOk={()=>setTotalsEditOpen(false)}>
          ...
          <div className="c-panel mb3">
            ...
            <div className="field">
              ...
              <div style={{fontSize:10,color:"var(--text4)",marginTop:4}}>Hospital = get from Baxter main hospital · Vendor = outside order needed · Both = can use either source</div>
            </div>
