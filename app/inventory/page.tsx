...
type AuditEvent = {
  ...
  staff: string;
  action:
    | "SCAN" | "LOOKUP_FOUND" | "LOOKUP_NOT_FOUND" | "ADD_ITEM"
    | "SUBMIT_TX" | "UNDO_TX" | "CHANGE_LOCATION" | "LOCK" | "UNLOCK"
    | "MAIN_OVERRIDE_ON" | "MAIN_OVERRIDE_OFF" | "SCANNER_OPEN" | "SCANNER_CLOSE"
    | "TOTALS_SET" | "TOTALS_ADJUST" | "AREA_LIST_LOAD" | "AREA_LIST_TOGGLE"
    | "AREA_ROW_EDIT_OPEN" | "AREA_ROW_EDIT_SAVE" | "ITEM_INACTIVE"
    | "ITEM_RESTORED" | "ITEM_STATUS_SAVE";
  details?: string;
};
...
const ITEM_STATUS_OPTIONS = [
  "IN STOCK","ORDERED","BACKORDER","PARTIAL","OUT OF STOCK","RECEIVED","CANCELLED",
] as const;

const LS = {
  PIN: "asc_pin_v1", AREA: "asc_area_id_v1", STAFF: "asc_staff_name_v1",
  AUDIT: "asc_audit_events_v1", LAST_TX: "asc_last_tx_v1",
};
  ...
  return (
    <div className="sp">
      {rows.map((row) => {
        ...
        return (
          <div key={row.id} className="ord-card" style={{ marginTop: 10 }}>
...
function PinSetter({ onSave }: { onSave: (pin: string) => void }) {
  ...
  return (
    <div>
      <input value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, "").slice(0, 6))} className="pin-inp" placeholder="New password" inputMode="numeric" type="password" />
      <button onClick={() => onSave(pin)} className="btn btn-submit btn-full btn-lg" style={{ marginTop: 14 }}>Save Password</button>
    </div>
  );
}
...
function ModeBtn({ active, danger, onClick, children }: { active: boolean; danger?: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className={`mode-btn ${danger ? "mode-use" : "mode-rst"} ${active ? "on" : "off"}`}>
      {children}
    </button>
  );
}
...
export default function InventoryPage() {
  ...
  const [pendingTotalsAction, setPendingTotalsAction] = useState<
    null|{kind:"SET";value:number}|{kind:"ADJUST";delta:number}|{kind:"SET_ACTIVE";is_active:boolean}
  >(null);
  ...
  const [pendingAreaRowSave, setPendingAreaRowSave] = useState<null|{
    storage_area_id:string; item_id:string; on_hand:number|null; par_level:number|null; low_level:number|null;
  }>(null);
  ...
  const filteredTotals = useMemo(() => {
    ...
    if (q) list = list.filter((r) => (r.name||"").toLowerCase().includes(q)||(r.vendor||"").toLowerCase().includes(q)||(r.category||"").toLowerCase().includes(q)||(r.reference_number||"").toLowerCase().includes(q)||(r.order_status||"").toLowerCase().includes(q));
    if (totalsLowOnly)  list = list.filter((r) => { const oh=r.total_on_hand??0; const low=r.low_level??0; return low>0&&oh<=low; });
    if (totalsZeroOnly) list = list.filter((r) => (r.par_level??0)===0||(r.low_level??0)===0);
    return list;
  ...
  const filteredAreaInv = useMemo(() => {
    ...
    let list = areaInv;
    if (q) list = list.filter((r) => (r.item_name||"").toLowerCase().includes(q)||(r.vendor||"").toLowerCase().includes(q)||(r.category||"").toLowerCase().includes(q)||(r.reference_number||"").toLowerCase().includes(q)||(r.order_status||"").toLowerCase().includes(q));
    if (areaParOnly) list = list.filter((r) => (r.par_level??0)>0);
    if (areaLowOnly) list = list.filter((r) => { const oh=r.on_hand??0; const low=r.low_level??0; return low>0&&oh<=low; });
    return list;
  ...
  useEffect(() => {
    try {
      ...
      const savedArea = localStorage.getItem(LS.AREA); if (savedArea) setAreaId(savedArea);
      setStaffName(localStorage.getItem(LS.STAFF) || "");
  ...
  function pushAudit(ev: Omit<AuditEvent,"id"|"ts"|"staff">) {
    ...
    setAudit((prev) => [{ id:uid(), ts:nowIso(), staff, action:ev.action, details:ev.details }, ...prev].slice(0,500));
  }
  ...
  async function loadTotals() {
    ...
    try {
      const { data, error } = await supabase.from("building_inventory_sheet_view").select("item_id,name,reference_number,vendor,category,total_on_hand,par_level,low_level,unit,notes,is_active,order_status,backordered").order("name",{ascending:true});
  ...
  async function loadAreaInventory() {
    ...
    try {
      const { data, error } = await supabase.from("storage_inventory_area_view").select("storage_area_id,storage_area_name,item_id,item_name,on_hand,par_level,low_level,unit,vendor,category,reference_number,notes,order_status,backordered").eq("storage_area_id",areaId).order("item_name",{ascending:true});
      if (error) throw error;
  ...
  async function startScanner() {
    ...
    setScannerOpen(true); pushAudit({ action:"SCANNER_OPEN", details:`Area=${selectedAreaName}` }); setStatus("Starting camera…");
    await new Promise((r) => setTimeout(r,80));
    if (!videoRef.current) { setStatus("Camera view not ready."); return; }
    stopScanner();
    try {
      const mod = await import("@zxing/browser"); const Reader = mod.BrowserMultiFormatReader; const reader = new Reader(); readerRef.current = reader;
      setStatus("Scanning…"); pushAudit({ action:"SCAN", details:`Area=${selectedAreaName}` });
      const constraints: MediaStreamConstraints = { video:{ facingMode:{ideal:"environment"}, width:{ideal:1920}, height:{ideal:1080} }, audio:false };
  ...
  function mapItemRow(r: any): Item {
    ...
    return { id:r.id, name:r.name, barcode:r.barcode??"", reference_number:r.reference_number??null, order_status:r.order_status??itemObj?.order_status??"IN STOCK", backordered:r.backordered??itemObj?.backordered??false };
  }

  async function lookup(queryRaw: string) {
    const q = queryRaw.trim(); if (!q) return;
    setItem(null); setOrderStatusRows([]); setMatches([]); setStatus("Looking up…");
    const res  = await fetch("/api/items/lookup", { method:"POST", headers:{"Content-Type":"application/json"}, cache:"no-store", body:JSON.stringify({query:q,mode:lookupMode}) });
    const json = await res.json();
    if (!json.ok) { setStatus(`Lookup failed: ${json.error}`); return; }
    if (json.item) { const it=mapItemRow(json.item); setItem(it); setItemStatusDraft({order_status:it.order_status||"IN STOCK",backordered:!!it.backordered}); setStatus(`Found: ${it.name}`); pushAudit({action:"LOOKUP_FOUND",details:`Item=${it.name} Mode=${lookupMode} Query=${q}`}); return; }
    const mapped = ((json.matches??[]) as any[]).map(mapItemRow);
    if (mapped.length) { setMatches(mapped); setStatus("Multiple matches — tap one."); return; }
    setStatus("NOT FOUND — Add Item"); pushAudit({action:"LOOKUP_NOT_FOUND",details:`Mode=${lookupMode} Query=${q}`}); setAddOpen(true); setAddName("");
  }
  ...
  async function suggest(queryRaw: string) {
    ...
    try {
      const res  = await fetch("/api/items/lookup", { method:"POST", headers:{"Content-Type":"application/json"}, cache:"no-store", body:JSON.stringify({query:q,mode:lookupMode,suggest:true}) });
      const json = await res.json();
  ...
  useEffect(() => {
    if (tab!=="Transaction"||areaListOpen||scannerOpen||addOpen||lookupMode==="BARCODE") { setSuggestLoading(false); return; }
    const t = window.setTimeout(()=>suggest(query),250); return ()=>window.clearTimeout(t);
  ...
  async function onPinConfirm() {
    ...
    if (pinPurpose==="changeLocation") { if (pendingArea) { const nextName=areas.find((a)=>a.id===pendingArea)?.name??pendingArea; setAreaId(pendingArea); pushAudit({action:"CHANGE_LOCATION",details:`To=${nextName}`}); } setPendingArea(""); return; }
    if (pinPurpose==="addItem") { await addItemNow(true); return; }
    if (pinPurpose==="totalsEdit") {
      const action=pendingTotalsAction; setPendingTotalsAction(null); if (!action||!totalsEditRow) return;
    ...
    if (pinPurpose==="areaRowEdit") { const payload=pendingAreaRowSave; setPendingAreaRowSave(null); if (!payload) return; await saveAreaRow(payload,true); return; }
    if (pinPurpose==="itemStatusEdit") { const payload=pendingItemStatusSave; setPendingItemStatusSave(null); if (!payload) return; await doSaveItemStatus(payload.item_id,payload.order_status,payload.backordered,payload.item_name); return; }
  }
  ...
  async function addItemNow(pinAlreadyPassed=false) {
    ...
    const barcode=query.trim(); if (!barcode) return alert("No barcode/value entered."); if (!addName.trim()) return alert("Enter item name."); if (!areaId) return alert("Select a location.");
    const res=await fetch("/api/items/create",{method:"POST",headers:{"Content-Type":"application/json"},cache:"no-store",body:JSON.stringify({name:addName.trim(),barcode,storage_area_id:areaId,par_level:addPar})});
    const json=await res.json(); if (!json.ok) return alert(`Add failed: ${json.error}`);
    const nextItem=mapItemRow(json.item); setItem(nextItem); setItemStatusDraft({order_status:nextItem.order_status||"IN STOCK",backordered:!!nextItem.backordered});
  ...
  async function fetchOnHand(area: string, itemId: string): Promise<number> {
    ...
    if (error) throw error; return (data?.on_hand??0) as number;
  }

  async function submit() {
    if (locked) return alert("Locked. Unlock first."); if (!staffName.trim()) { setTab("Audit"); return alert("Enter staff name in Audit tab first."); }
    if (!item?.id) return alert("Find or scan an item first."); if (!areaId) return alert("Select a location.");
    try {
      if (mode==="USE") {
        const useArea=mainOverride?MAIN_SUPPLY_ID:areaId;
        const {error}=await supabase.rpc("use_stock",{p_item_id:item.id,p_area_id:useArea,p_qty:qty}); if (error) throw error;
        ...
        if (tab==="Totals") await loadTotals(); if (tab==="Transaction"&&areaListOpen) await loadAreaInventory();
        pushAudit({action:"SUBMIT_TX",details:`Mode=USE Qty=${qty} Item=${item.name} Area=${useArea===MAIN_SUPPLY_ID?"MAIN SUPPLY":selectedAreaName} Override=${mainOverride?"MAIN":"NO"}`}); return;
      }
      if (mode==="RESTOCK") {
        const {error}=await supabase.rpc("move_stock",{p_item_id:item.id,p_from_area:MAIN_SUPPLY_ID,p_to_area:areaId,p_qty:qty}); if (error) throw error;
        ...
        if (tab==="Totals") await loadTotals(); if (tab==="Transaction"&&areaListOpen) await loadAreaInventory();
        pushAudit({action:"SUBMIT_TX",details:`Mode=RESTOCK Qty=${qty} Item=${item.name} From=MAIN SUPPLY To=${selectedAreaName}`}); return;
      }
    } catch (e: any) { alert(`Transaction failed: ${e?.message??"unknown error"}`); }
  }

  async function undoLast() {
    if (!lastTx||undoBusy||locked) return; if (!staffName.trim()) { setTab("Audit"); return alert("Enter staff name in Audit tab first."); }
    ...
    if (!ok) return; setUndoBusy(true);
    try {
      if (lastTx.mode==="USE") {
        const {error}=await supabase.rpc("add_stock",{p_item_id:lastTx.item_id,p_area_id:lastTx.storage_area_id,p_qty:lastTx.qty}); if (error) throw error;
        const newOnHand=await fetchOnHand(lastTx.storage_area_id,lastTx.item_id); setStatus(`↩️ UNDONE. On-hand now ${newOnHand}`);
        ...
        setLastTx(null); if (tab==="Totals") await loadTotals(); if (tab==="Transaction"&&areaListOpen) await loadAreaInventory(); return;
      }
      if (lastTx.mode==="RESTOCK") {
        const {error}=await supabase.rpc("move_stock",{p_item_id:lastTx.item_id,p_from_area:lastTx.storage_area_id,p_to_area:MAIN_SUPPLY_ID,p_qty:lastTx.qty}); if (error) throw error;
        const newOnHand=await fetchOnHand(lastTx.storage_area_id,lastTx.item_id); setStatus(`↩️ UNDONE. On-hand now ${newOnHand}`);
    ...
    } catch (e: any) { alert(`Undo failed: ${e?.message??"unknown error"}`); } finally { setUndoBusy(false); }
  }

  function savePin(newPin: string) {
    const cleaned=newPin.replace(/\D/g,"").slice(0,6); if (cleaned.length<4) return alert("PIN must be at least 4 digits.");
    localStorage.setItem(LS.PIN,cleaned); alert("PIN saved ✅");
  }

  function onToggleOverride() {
    setMainOverride((v) => { const next=!v; pushAudit({action:next?"MAIN_OVERRIDE_ON":"MAIN_OVERRIDE_OFF",details:`Area=${selectedAreaName}`}); return next; });
  }

  async function loadOrderRowsForItem(itemId: string, mode2: "modal"|"totals"="modal") {
    if (mode2==="modal") { setOrderStatusLoading(true); setOrderStatusRows([]); } else { setTotalsOrderLoading(true); setTotalsOrderRows([]); }
    try {
      const {data,error}=await supabase.from("purchase_order_items").select("id,qty_ordered,qty_received,status,notes,purchase_orders(id,po_number,vendor,status,expected_date,order_date,notes)").eq("item_id",itemId).order("created_at",{ascending:false});
      if (error) throw error;
      if (mode2==="modal") setOrderStatusRows((data as OrderStatusRow[])??[]); else setTotalsOrderRows((data as OrderStatusRow[])??[]);
    } catch (e: any) {
      if (mode2==="modal") { alert(`Order status failed: ${e?.message??"unknown error"}`); setOrderStatusOpen(false); } else setTotalsOrderRows([]);
    } finally { if (mode2==="modal") setOrderStatusLoading(false); else setTotalsOrderLoading(false); }
  }
  ...
  function parseIntSafe(raw: string): number|null {
    const cleaned=raw.trim(); if (!cleaned||!/^-?\d+$/.test(cleaned)) return null; const n=Number(cleaned); return Number.isFinite(n)?n:null;
  }
  ...
  async function doTotalsSet(row: BuildingTotalRow, value: number, pinAlreadyPassed=false) {
    ...
    const json=await res.json(); if (!json.ok) { alert(`Update failed: ${json.error}`); return; }
    pushAudit({action:"TOTALS_SET",details:`Item=${row.name} Set=${value}`}); setTotalsEditOpen(false); await loadTotals();
  }
  ...
  async function doTotalsAdjust(row: BuildingTotalRow, delta: number, pinAlreadyPassed=false) {
    ...
    const json=await res.json(); if (!json.ok) { alert(`Update failed: ${json.error}`); return; }
    pushAudit({action:"TOTALS_ADJUST",details:`Item=${row.name} Delta=${delta>0?"+":""}${delta}`}); setTotalsEditOpen(false); await loadTotals();
  }
  ...
  async function doTotalsSetActive(row: BuildingTotalRow, is_active: boolean, pinAlreadyPassed=false) {
    ...
    if (!confirm(is_active?`Restore "${row.name}" to active items?`:`Move "${row.name}" to inactive items?`)) return;
    ...
    const json=await res.json(); if (!json.ok) { alert(`Update failed: ${json.error}`); return; }
    pushAudit({action:is_active?"ITEM_RESTORED":"ITEM_INACTIVE",details:`Item=${row.name}`}); setTotalsEditOpen(false); await loadTotals();
  }
  ...
  async function doSaveItemStatus(itemId: string, order_status: string, backordered: boolean, itemName?: string) {
    ...
    try {
      ...
      const {error}=await supabase.from("items").update({order_status:cleanStatus,backordered:!!backordered}).eq("id",itemId); if (error) throw error;
    ...
    } catch (e: any) { alert(`Status save failed: ${e?.message??"unknown error"}`); } finally { setItemStatusSaving(false); }
  }
  ...
  function openAreaRowEditor(row: AreaInvRow) {
    setAreaEditRow(row); setAreaEditOnHand(String(row.on_hand??0)); setAreaEditPar(String(row.par_level??0)); setAreaEditLow(String(row.low_level??0));
    setAreaEditOpen(true); pushAudit({action:"AREA_ROW_EDIT_OPEN",details:`Area=${row.storage_area_name} Item=${row.item_name}`});
  }
  ...
  async function saveAreaRow(payload: {storage_area_id:string;item_id:string;on_hand:number|null;par_level:number|null;low_level:number|null}, pinAlreadyPassed=false) {
    if (!staffName.trim()) { setTab("Audit"); alert("Enter staff name in Audit tab first."); return; }
    ...
    const json=await res.json(); if (!json.ok) { alert(`Save failed: ${json.error}`); return; }
                ...
                <div className="c-inner mt3">
                  ...
                  {areaEditOpen && areaEditRow && (
                    <AscModal title="Edit area item" okText="Save" onCancel={()=>setAreaEditOpen(false)} onOk={async()=>{try{const payload=buildAreaSavePayload(areaEditRow);await saveAreaRow(payload);}catch(e:any){alert(e?.message??"Invalid values");}}}>
                      ...
                      <div className="g3">
                        <div className="field">
                          <label className="f-lbl">On hand</label>
                          <input value={areaEditOnHand} onChange={(e)=>setAreaEditOnHand(e.target.value.replace(/[^\d]/g,""))} inputMode="numeric" className="inp" style={{textAlign:"center"}} />
                        </div>
                        <div className="field">
                          <label className="f-lbl">PAR</label>
                          <input value={areaEditPar} onChange={(e)=>setAreaEditPar(e.target.value.replace(/[^\d]/g,""))} inputMode="numeric" className="inp" style={{textAlign:"center"}} />
                        </div>
                        <div className="field">
                          <label className="f-lbl">LOW</label>
                          <input value={areaEditLow} onChange={(e)=>setAreaEditLow(e.target.value.replace(/[^\d]/g,""))} inputMode="numeric" className="inp" style={{textAlign:"center"}} />
                        </div>
                      </div>
                ...
                {filteredTotals.map((r)=>{
                  ...
                  return (
                    <button key={`${r.item_id}-${r.reference_number??""}`} onClick={()=>openTotalsEditor(r)} className={`item-card ${isLow?"low":zeroSetup?"warn":"ok"}`}>
                      ...
                      <div className="stats-row">
                        {[{label:"Par",value:par,warn:par===0},{label:"Low",value:low,warn:low===0},{label:"Unit",value:r.unit??"—",warn:false}].map(({label,value,warn})=>(
          ...
          {tab==="Audit" && (
            <div className="c-card anim">
              ...
              <div className="field">
                ...
                <input value={staffName} onChange={(e)=>setStaffName(e.target.value)} className="inp" placeholder="e.g., Jeremy Johnson" />
              </div>
              ...
              <div className="sp">
                {audit.length===0?(
                  <div className="c-deep" style={{textAlign:"center",color:"var(--text3)",fontSize:13,padding:"20px 0"}}>No audit events yet.</div>
                ):(
