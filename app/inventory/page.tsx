...
export default function InventoryPage() {
  ...
  async function loadLocations(){setAreasLoading(true);try{const{data,error}=await supabase.from("storage_areas").select("id,name").order("name");if(error||!data){setAreas([]);setAreaId("");return;}const list:Area[]=data.map((a:any)=>({id:a.id,name:a.name}));setAreas(list);setAreaId((prev)=>{if(!list.length)return"";return list.some((a)=>a.id===prev)?prev:list[0].id;});setStatus("");}catch(e:any){setStatus(`Locations fetch failed: ${e?.message??"unknown"}`);setAreas([]);setAreaId("");}finally{setAreasLoading(false);}}

  useEffect(()=>{loadLocations();},[]);// eslint-disable-line

  async function loadTotals(retriesLeft=2){setTotalsLoading(true);setTotalsError("");try{const{data,error}=await supabase.from("building_inventory_sheet_view").select("item_id,name,reference_number,vendor,category,total_on_hand,par_level,low_level,unit,notes,is_active,order_status,backordered").order("name",{ascending:true});if(error)throw error;const rows=(data as BuildingTotalRow[])??[];const ids=rows.map((r)=>r.item_id);if(ids.length>0){const[srcRes,orderRes]=await Promise.all([supabase.from("items").select("id,supply_source,price,expiration_date,alert_note").in("id",ids),supabase.from("order_requests").select("item_id,created_at,status").in("status",["PENDING","ORDERED"]).in("item_id",ids).order("created_at",{ascending:false})]);const srcData=srcRes.data;const orderData=orderRes.data;if(srcData){const map=Object.fromEntries(srcData.map((r:any)=>[r.id,r]));rows.forEach((r)=>{r.supply_source=map[r.item_id]?.supply_source??null;r.price=map[r.item_id]?.price??null;r.expiration_date=map[r.item_id]?.expiration_date??null;r.alert_note=map[r.item_id]?.alert_note??null;});}if(orderData){const orderMap:Record<string,{date:string,status:string}>={};orderData.forEach((o:any)=>{if(!orderMap[o.item_id])orderMap[o.item_id]={date:o.created_at,status:o.status};});rows.forEach((r)=>{r.ordered_at=orderMap[r.item_id]?.date??null;if(orderMap[r.item_id])r.order_status=orderMap[r.item_id].status;});}}setTotals(rows);setTotalsLoading(false);}catch(e:any){if(retriesLeft>0){await new Promise((res)=>setTimeout(res,1200));return loadTotals(retriesLeft-1);}setTotals([]);setTotalsError(e?.message??"Failed to load totals");setTotalsLoading(false);}}
