...
export default function InventoryPage() {
  ...
  async function loadTotals(){setTotalsLoading(true);setTotalsError("");try{const[{data,error},]=await Promise.all([supabase.from("building_inventory_sheet_view").select("item_id,name,reference_number,vendor,category,total_on_hand,par_level,low_level,unit,notes,is_active,order_status,backordered").order("name",{ascending:true})]);if(error)throw error;const rows=(data as BuildingTotalRow[])??[];const ids=rows.map((r)=>r.item_id);if(ids.length>0){const{data:srcData}=await supabase.from("items").select("id,supply_source,price,expiration_date,alert_note").in("id",ids);if(srcData){const map=Object.fromEntries(srcData.map((r:any)=>[r.id,r]));rows.forEach((r)=>{r.supply_source=map[r.item_id]?.supply_source??null;r.price=map[r.item_id]?.price??null;r.expiration_date=map[r.item_id]?.expiration_date??null;r.alert_note=map[r.item_id]?.alert_note??null;});}
          // Fetch latest order dates for items on order
          const {data:orderData}=await supabase.from("order_requests").select("item_id,created_at").in("status",["PENDING","ORDERED"]).in("item_id",ids).order("created_at",{ascending:false});
          if(orderData){const orderMap:Record<string,string>={};orderData.forEach((o:any)=>{if(!orderMap[o.item_id])orderMap[o.item_id]=o.created_at;});rows.forEach((r)=>{r.ordered_at=orderMap[r.item_id]??null;});}}setTotals(rows);}catch(e:any){setTotals([]);setTotalsError(e?.message??"Failed to load totals");}finally{setTotalsLoading(false);}}
