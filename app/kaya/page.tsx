...
export default function KayaRestockPage() {
  const router = useRouter();
  useSessionTimeout();

  const [items, setItems] = useState<Item[]>([]);
  const [requests, setRequests] = useState<RestockRequest[]>([]);
  const [staffName, setStaffName] = useState("Kaya");
  ...
  async function loadData(background = false) {
    ...
    try {
      ...
      setItems((inventory.data || []).map((row: any) => ({
        item_id: row.item_id,
        name: row.name,
      ...
      setRequests(
        (requestJson.data || [])
          .filter((request: RestockRequest) => request.requested_from === KAYA_AREA)
          .slice(0, 40)
      );
    } catch (err: any) {
      setError(err?.message || "The inventory list could not be loaded.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    try {
      ...
      const response = await fetch("/api/restock-request", {
        ...
        body: JSON.stringify({
          ...
          requested_by: staffName || "Kaya",
          requested_from: KAYA_AREA,
        }),
      });
      ...
      setToast({ type: "err", text: err?.message || "The restock request could not be sent." });
    } finally {
      setSubmitting(false);
      ...
      <main className="k-root">
        <div className="k-header">
          <div className="k-header-in">
            ...
            <div style={{ flex: 1 }}>
              <div className="k-title">Kaya Restock</div>
              <div className="k-sub">Case-picking restock requests · live inventory</div>
            </div>
        ...
        <div className="k-wrap">
          ...
          {requests.length > 0 && (
            <section className="k-recent">
              <div className="k-section-title">Kaya’s Recent Requests</div>
