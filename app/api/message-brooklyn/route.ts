...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabaseAdmin.from("brooklyn_messages").select("id,created_at,sender_name,subject,message,delivery_status").order("created_at", { ascending: false }).limit(50);
...
export async function POST(request: Request) {
  try {
    ...
    }).select("id,created_at,sender_name,subject,message,delivery_status").single();
